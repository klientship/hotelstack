<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <div class="demo-alignment">
      <vs-popup classContent="popup-example" title="Create Floor" :active.sync="isAddNewPopup">
        <!-- ERRORS -->
          ...
          <div class="p-4">
            <div class="vx-row mb-4">
              <div class="vx-col w-full">
                <vs-input class="w-full" label-placeholder="Floor Name" v-model="form.name" />
              </div>
            </div>
            <div class="vx-row mb-4">
              <div class="vx-col w-full">
                <vs-input
                  type="text"
                  class="w-full"
                  label-placeholder="Floor Number"
                  v-model="form.number"
                />
              </div>
            </div>
            ...
            <div class="vx-row">
              <div class="vx-col w-full">
                ...
                <vs-button
                  ...
                  type="border"
                  class="mb-4"
                  @click="form.name = form.number = form.description = ''; form.status= 1;"
                >Reset</vs-button>
              </div>
            </div>
          </div>
    ...
    <!-- update -->
    <div class="demo-alignment">
      <vs-popup classContent="popup-example" title="Edit Floor" :active.sync="isUpdatePopup">
        <!-- ERRORS -->
        <display-error :form="form"></display-error>
        <!-- End ERRORS -->
...
export default {
  ...
  computed: {
    ...
    products() {
      return this.$store.getters.getAllFloors;
    },
    queriedItems() {
  ...
  methods: {
    ...
    onSubmit() {
      this.form
        ...
        .then(() => {
          this.$store.dispatch("retrieveFloors");
          this.$vs.notify({
            color: "success",
            title: "Created",
            text: "The floor created successfully.",
          });
        })
        .catch();
    },
    deleteData(id) {
      ...
      // this.$store.dispatch("deleteFloor", id).then(
      //   this.$vs.notify({
      //     color: "danger",
      //     title: "Deleted",
      //     text: "The selected floor was deleted successfully.",
      //   })
      // );
      this.$store.dispatch("retrieveFloors");
    },
    editData(data) {
      // this.$router.push({ name: "floors.edit", params: { floor_id: data.id } });\
    ...
    onUpdate() {
      this.form
        .submit("patch", `/api/floors/${this.form.id}`)
        .then(() => {
          this.$store.dispatch("retrieveFloors");
          this.$vs.notify({
            color: "success",
            title: "Updated",
            text: "The floor updated successfully.",
          });
        })
        .catch();
    },
    ...
    deleteSelected() {
      ...
      //     this.$store.dispatch("deleteFloor", item.id).then(
      //       this.$vs.notify({
      //         color: "danger",
      //         title: "Deleted",
      //         text: "The selected floors was deleted successfully.",
      //       })
      //     );
      //   });
      //   this.$store.dispatch("retrieveFloors");
