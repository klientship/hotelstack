        ...
        <div class="mb-4 md:mb-0 mr-4 ag-grid-table-actions-left">
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer">
            ...
            >
              <span
                class="mr-2"
              >{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ payments.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : payments.length }} of {{ payments.length }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
...
export default {
  ...
  computed: {
    ...mapGetters({ payments: "getAllPayments" }),
  ...
  methods: {
    thisMonth() {
      this.$store.dispatch("RETRIEVE_THIS_MONTH_PAYMENTS");
      this.filter = "this_month";
    },
    lastMonth() {
      this.$store.dispatch("RETRIEVE_LAST_MONTH_PAYMENTS");
      this.filter = "last_month";
    },
    all() {
      this.$store.dispatch("RETRIEVE_ALL_PAYMENTS");
      this.filter = "all";
    },
