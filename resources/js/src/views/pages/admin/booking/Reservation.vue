<template>
  <div id="data-list-list-view" class="data-list-container">
            ...
            <vs-td class="whitespace-no-wrap">
              ...
              <vs-dropdown vs-trigger-click class="dd-actions cursor-pointer mr-4 mb-4">
                ...
                <vs-dropdown-menu>
                  <vs-dropdown-item
                    @click="no_show(tr.id)"
                    v-if="!tr.no_show && !tr.checked_in && !tr.checked_out "
                  >
                    <span class="flex items-center">
    ...
    <div class="demo-alignment">
      ...
      >
        <CreateBooking title="Reservation"></CreateBooking>
      </vs-popup>
    </div>
    ...
    <!-- update -->
    <div class="demo-alignment">
      <vs-popup classContent="popup-example" title="Edit Reservation" :active.sync="isUpdatePopup">
        <EditBooking title="Reservation" :data="form"></EditBooking>
      </vs-popup>
    </div>
    ...
    <div class="demo-alignment">
      <vs-popup classContent="popup-example" title="Paid Service" :active.sync="isPaidServicePopup">
        <PaidService title="Reservation" :id="form.id"></PaidService>
      </vs-popup>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    no_show(id) {
      this.$store.dispatch("NO_SHOW", id).then((res) => {
        ...
        click: () => {
          this.$store.dispatch("deleteReservation", id).then(() => {
            ...
            this.$vs.notify({
              color: "danger",
              title: "Deleted",
              text: "The selected Reservation deleted successfully.",
            });
          });
        },
      });
    ...
    getStatusColor(data) {
      ...
      if (data.no_show && !data.checked_in && !data.checked_out) return "dark";
    ...
    getStatus(data) {
      if (data.no_show && !data.checked_in && !data.checked_out)
        return "NO SHOW";
    ...
    deleteSelected() {
      if (this.selected.length > 0) {
        this.selected.forEach((item) => {
          this.$store.dispatch("deleteReservation", item.id).then(
            this.$vs.notify({
              color: "danger",
              title: "Deleted",
              text: "The selected Reservation was deleted successfully.",
            })
          );
        });
        this.$store.dispatch("retrieveReservations");
        ...
        this.$vs.notify({
          color: "warning",
          title: "Warning",
          text: "Please select Reservation to delete.",
        });
      }
    },
